<html>
<head>
	<meta charset="UTF-8">
	<title>Home Page</title>
	
	
	
		<link rel="stylesheet" href="css/style.css">

	
</head>
<body>
	<div class="wrapper">
		<div class="container">
		<h1>Home Page</h1>
			<form action="user-profile.php">
         		<button type="submit">Update Profile</button>
			</form>
			 <form action="user_password.php">
				<button type="submit">Update Password</button>
            </form>
			<form action="user_mobile.php">
				<button type="submit">Add Mobile Number</button>
			</form>
		    <form action="org_new_listing.php">
         		<button type="submit">Add New Listing</button>
			</form>
			<form action="org_view_listing.php">
				<button type="submit">View My Listings</button>
			</form>
			<!--form action="org_applied.php">
				<button type="submit">View Applicants</button>
            </form-->
			<form action="org_filter.php">
				<button type="submit">Select Serviceman</button>
            </form>
			<form action="org_view_upcoming.php">
				<button type="submit">View Alloted Services</button>
            </form>
			
			<form action="review.php">
				<button type="submit">Give Reviews</button>
            </form>
            <!-- <form action="org_view_profile.php">
				<button type="submit">View My Profile</button>
            </form>            -->
			 
			<form action="index.php">
				<button type="submit">Log Out</button>
			</form>
      </div>
		
		<ul class="bg-bubbles">
			<li></li>
			<li></li>
			<li></li>
			<li></li>
			<li></li>
			<li></li>
			<li></li>
			<li></li>
			<li></li>
			<li></li>
		</ul>
	</div>
	<!-- <br><br>
	<center>
		<h1>Signup</h1>
		<form action = "signup-backend.php" method = "POST">
			<input name = "name" placeholder = "Name"><br><br>
			<input name = "email" placeholder = "Email"><br><br>
			<input name = "password" type = "password" placeholder = "Password"><br><br>
			<button type = "submit">Submit</button>
		</form>
	</center> -->
</body>
</html>
